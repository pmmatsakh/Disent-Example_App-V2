# mysql database example — the CRUD sample script, with a browser UI.
#
# MySQL / MariaDB · relational database · CREATE / INSERT / SELECT / UPDATE / DELETE
#
...
#
# Binds 0.0.0.0:8008 (§8). my-container-8008.example.com -> :8008.
# NOTE: :8008 is a PROPOSED port — confirm with Anthony.
...
def connect():
    ...
    dsn = os.environ.get("MYSQL_DSN", "mysql://disent:disent@127.0.0.1:3306/disent")
    u = urlparse(dsn)
    conn = pymysql.connect(host=u.hostname or "127.0.0.1", port=u.port or 3306,
                           user=u.username or "disent", password=u.password or "disent",
                           database=(u.path or "/disent").lstrip("/"),
...
def server_version(conn) -> str:
    ...
    try:
        ...
        return "mysql (version unavailable)"
...
def run_demo():
    ...
    count INT NOT NULL DEFAULT 0
        ...
        step("CREATE a table", "CREATE TABLE fruit (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(64) UNIQUE, count INT)", "table created")
...
def root() -> str:
    ...
    return PAGE.format(app=APP_NAME, sub='MySQL / MariaDB · relational database · CREATE / INSERT / SELECT / UPDATE / DELETE', note=note, body=body,
                       host=HOST, port=PORT)
