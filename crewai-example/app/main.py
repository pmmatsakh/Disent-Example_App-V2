# crewai AI example — CrewAI - role-based agents - Agent → Task → Crew.kickoff()
#
# CrewAI multi-agent crew: a researcher agent and a writer agent run in sequence.
#
...
#
# Binds 0.0.0.0:3404 (§8). my-container-3404.example.com -> :3404.
# NOTE: :3404 is a PROPOSED port (AI 3400s block) — confirm with Anthony.
...
APP_NAME = "crewai"
PORT = int(os.environ.get("PORT", 3404))
...
def run_agent(topic: str, api_key: str):
    ...
    # CrewAI routes to any provider via a "provider/model" string.
    llm = LLM(model="anthropic/claude-opus-4-8", api_key=api_key)

    researcher = Agent(
        role="Researcher",
        goal="Find the single most interesting fact about the topic.",
        backstory="You are concise and precise. You never pad your answers.",
        llm=llm, verbose=False,
    )
    writer = Agent(
        role="Writer",
        goal="Turn the research into one vivid sentence a beginner understands.",
        backstory="You write plainly, with no jargon and no preamble.",
        llm=llm, verbose=False,
    )
    research = Task(
        description=f"Find one surprising fact about: {topic}",
        expected_output="A single factual sentence.", agent=researcher,
    )
    write = Task(
        description="Rewrite the researcher's fact as one vivid beginner-friendly sentence.",
        expected_output="One sentence, no preamble.", agent=writer,
    )
...
def root() -> str:
    if KEY_VAR is None:
        ...
        keyfield = KEYFIELD
    return PAGE.format(app=APP_NAME, blurb='CrewAI · role-based agents · Agent → Task → Crew.kickoff()', status=status,
