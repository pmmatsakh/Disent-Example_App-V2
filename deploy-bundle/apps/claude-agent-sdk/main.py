# claude_agent_sdk AI example — Claude Agent SDK - Claude Code as a library - query()
#
# Anthropic's Claude Agent SDK — the Claude Code harness as an importable library.
#
...
# typed into the UI is passed only to that request's isolated subprocess and
# never stored.
#
# Binds 0.0.0.0:3407 (§8). my-container-3407.example.com -> :3407.
# NOTE: :3407 is a PROPOSED port (AI 3400s block) — confirm with Anthony.
...
APP_NAME = "claude_agent_sdk"
PORT = int(os.environ.get("PORT", 3407))
...
def run_agent(topic: str, api_key: str):
    ...
    async def _go():
        opts = ClaudeAgentOptions(
            system_prompt="Answer in one short sentence. No preamble.",
            max_turns=1,
            tools=[],
            allowed_tools=[],   # no filesystem/bash for a hello-world tile
            setting_sources=[], # do not load user/project config or CLAUDE.md
            env={KEY_VAR: api_key},  # request-local subprocess environment
        )
        chunks = []
        async for message in query(prompt=topic, options=opts):
...
def root() -> str:
    if KEY_VAR is None:
        ...
        keyfield = KEYFIELD
    return PAGE.format(app=APP_NAME, blurb='Claude Agent SDK · Claude Code as a library · query()', status=status,
