# openai_agent_sdk AI example — OpenAI Agents SDK - Agent + Runner.run()
#
# OpenAI's Agents SDK: define an Agent, run it with Runner.
#
...
#
# Binds 0.0.0.0:3408 (§8). my-container-3408.example.com -> :3408.
# NOTE: :3408 is a PROPOSED port (AI 3400s block) — confirm with Anthony.
...
APP_NAME = "openai_agent_sdk"
PORT = int(os.environ.get("PORT", 3408))
...
def run_agent(topic: str, api_key: str):
    ...
    # A request-local client keeps concurrent users' keys isolated. Do not put
    # the key in os.environ: that process-wide state is shared by all requests.
    async def _go():
        async with AsyncOpenAI(api_key=api_key) as client:
            model = OpenAIResponsesModel(model="gpt-4.1", openai_client=client)
            agent = Agent(
                name="Assistant",
                instructions="Answer in one short sentence. No preamble.",
                model=model,
            )
...
def root() -> str:
    if KEY_VAR is None:
        ...
        keyfield = KEYFIELD
    return PAGE.format(app=APP_NAME, blurb='OpenAI Agents SDK · Agent + Runner.run()', status=status,
